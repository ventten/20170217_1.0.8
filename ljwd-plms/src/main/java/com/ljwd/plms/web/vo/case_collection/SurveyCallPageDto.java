 ...
 * 呼叫记录分页Dto
 * */
public class SurveyCallPageDto extends PageRowNo {
    ...
    private Integer tableFrom;              //数据来源:1-微贷 LN_SURVEY_CALL、2-贷后 PLMS_LN_SURVEY_CALL
    ...
    public String getCloudcallStatusText() {
        if(cloudcallStatus == null){
            if(callLong == null){
                return "失败";
            }else if("0".equals(callLong)){
                return "未接通";
            }else {
                return "接通";
            }
        }else{
            //0-14是微贷云呼叫的状态、15-17、BUSY是贷后明元呼叫的状态
            if("0".equals(cloudcallStatus)){
                return "呼叫请求成功";
