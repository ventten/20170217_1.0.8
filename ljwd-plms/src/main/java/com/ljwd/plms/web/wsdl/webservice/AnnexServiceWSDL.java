...
import com.ljwd.plms.web.wsdl.webservice.annexService.AnnexService;
import com.ljwd.plms.web.wsdl.webservice.annexService.AnnexServiceService;
 ...
 */
public class AnnexServiceWSDL {

    private static Logger logger = LoggerFactory.getLogger(AnnexServiceWSDL.class);
    ...
    /** 微贷系统连接实例代理 */
    private static AnnexService proxyService = null;

    /** 微贷系统登录用户信息 */
    private static com.ljwd.plms.web.wsdl.webservice.channelService.UserProfileDto userProfileDto = null;
    ...
    static {
        ...
        String urlString = ConfigUtil.getStringVal("interface.config.mfbms.app.web.annex.url");
        try {
     ...
     * @return 返回连接代理
     */
    public static AnnexService getService(){
        if (proxyService == null) {
            synchronized (synchronizedObject) {
                ...
                // 创建连接
                AnnexService originalService = createOriginalAnnexService();
                // 创建掉线重连代理处理器
     ...
     * @return 返回代理后的对象
     */
    protected static AnnexService bindHandler(AnnexService originalService, InvocationHandler handler) {
        login(originalService); // 登录微贷系统
        // 绑定掉线重连代理,并返回代理对象
        return (AnnexService) Proxy.newProxyInstance(AnnexService.class.getClassLoader(), new Class<?>[]{AnnexService.class}, handler);
    }
     ...
     */
    protected static void login(AnnexService originalService) {
        try {
            // 登录微贷系统
     ...
     * @return 返回微贷连接对象
     */
    protected static AnnexService createOriginalAnnexService() {
        logger.info("连接微贷annex服务url:" + url);
        AnnexService originalService = new AnnexServiceService(url).getAnnexServicePort();
     ...
     * @return 返回登录用户信息
     */
    public com.ljwd.plms.web.wsdl.webservice.channelService.UserProfileDto getUserProfileDto() {
    ...
    private static class ReLoginInvocationHandler implements InvocationHandler {
        /** 被代理的原始对象 */
        private AnnexService originalService;

        public ReLoginInvocationHandler(AnnexService target) {
        ...
        public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
            try {
                ...
                /** 错误信息查看类的类注释
                 * @see com.ljwd.plms.web.wsdl.webservice.AnnexServiceWSDL */
                ...
                if (cause != null && (msg = cause.getLocalizedMessage()) != null && msg.indexOf("MFBMS_SYS_001103") != -1) {
                    ...
                    // 重新连接微贷系统并再次执行本次业务请求
                    synchronized (synchronizedObject) {
                        originalService = createOriginalAnnexService();
